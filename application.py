...
try:
    mongo_uri = "mongodb+srv://Joel:FParad0x!@cluster0.fq7mbcx.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    client = MongoClient(mongo_uri)
    db = client["income_survey"]
    collection = db["responses"]
    print("[LOG] Connected to MongoDB Atlas.")
except Exception as e:
    print("[ERROR] Could not connect to MongoDB Atlas:", str(e))
    collection = None
    ...
    if request.method == "POST":
        try:
            ...
            if collection is not None:
                collection.insert_one(user.to_mongo_doc())
                print("[LOG] Record also inserted into MongoDB Atlas.")

            return "<h3>Thank you! Your response has been recorded.</h3><a href='/'>Submit another response</a>"
